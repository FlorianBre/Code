...
#include <msp430.h>
#include <lib\DMA.h>
void DMAinit0(unsigned int trigSrc,  unsigned int *srcAdr,  volatile unsigned int *desAdr, unsigned int srcIncr, unsigned int desIncr, unsigned int transferSize, unsigned int interruptEnable ){
    //Source adress
    ...
    DMA0CTL = DMADT_6 | srcIncr | desIncr | interruptEnable | DMAEN;
}

void DMAinit1(unsigned int trigSrc, unsigned int *srcAdr, volatile unsigned int *desAdr, unsigned int srcIncr, unsigned int desIncr, unsigned int transferSize, unsigned int interruptEnable ){
    ...
    DMACTL0 |= trigSrc;
    DMA1SZ = transferSize;
    DMA1CTL = DMADT_6 | srcIncr | desIncr | interruptEnable | DMAEN;
}
...
void DMAinit2(unsigned int trigSrc,  volatile unsigned int *srcAdr, volatile unsigned int *desAdr, unsigned int srcIncr, unsigned int desIncr, unsigned int transferSize, unsigned int interruptEnable){
    ...
    DMACTL1 = trigSrc;
    DMA2SZ = transferSize;
    DMA2CTL = DMADT_6 | srcIncr | desIncr | interruptEnable | DMAEN;
}
