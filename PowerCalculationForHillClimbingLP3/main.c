 ...
 * P2.1: ADCinit Output Signal TB0 (Control Signal to start SH circuit, Set High to start)
...
void init(){
    ...
    // Timer For Setting ADCinit high after wakeUpTime
    timerInitPWMB0(wakeUpTime, TASSEL_3, 0.5, OUTMOD_3);

    // Configure Pin as TB0 Trigger
    P3DIR &= ~BIT3;
    P3SEL1 = BIT3;
    P3SEL0 = BIT3;
    timerInitPWMA1(200, TASSEL_3,0,OUTMOD_0);
    TA1CCR1 = 1;
 ...
 * P1.1: Sample Hold ready Input Triggers TA1 (Control Input Signal, high when SH circuit is ready for ADC mesasurement)
 * P1.7: Capture Compare Input store timer Value in CCR2 timer A0 (measure T_off)
 * P1.6: TimerStart, input (Starts timer A0 on a Rising Edge off PWM input Signal)
 * P4.7: Controll bit output, toggle bit for testing
 * P9.0: ADC1 input Ucaphigh
...
void init(){
    ...
    // init ADCSequence Pin P9.0,P9.1,P9.2.
    adcInitSequence(ADC12SSEL_3, 0, ADC12SHT0_0, ADC12VRSEL_0, 0, 0, ADC12SHS_4, ADC12INCH_8,  ADC12INCH_10);
    //REFCTL0 |= REFON | REFVSEL_2;   // Turn on internal Reference Generator, select Reference
    //while( REFCTL0 & REFGENBUSY){ };    // Wait for refernce to settle
    //ADC12MCTL0 |= ADC12VRSEL_1;
    //ADC12MCTL1 |= ADC12VRSEL_1;
